...
class MultiUnityWrapper():
    ...
    def __init__(
        ...
        for behaviour_name, group_spec in self._env.behavior_specs.items():
            print("This is group_spec:", group_spec)
            # Set observations space
            if group_spec.action_spec.is_discrete():
                ...
                if group_spec.action_spec.discrete_size == 1:
                    ...
                    print("This is the action_space type:", type(action_space))
                else:
                    if flatten_branched:
                        self._flattener = ActionFlattener(branches)
    ...
    def step(self, action_dict: Dict) -> MultiStepResult:
        ...
        print("This is the action_dict:", action_dict)
        for agent_id, action in action_dict.items():
            behaviour_name = self._agent_id_to_behaviour_name[agent_id]
            print("THIS IS THE BEHAVIOUR NAME:", behaviour_name)
            print("This is the agent_id:", agent_id)
            print("This is the action:", action)
            action_tuple = ActionTuple(discrete = np.array([[action]]))
            # print("This is action.continuous.shape:", action_tuple.continuous.shape)
            # print("This is action.discrete.shape:", action_tuple.discrete.shape)
